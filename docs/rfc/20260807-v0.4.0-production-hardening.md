...
- **Status:** `ACCEPTED` *(Disetujui via "Gasskan" 2026-08-07 — sedang dieksekusi)*
...
### Batch 1: Verification Harness & Contract Locking
* **DependsOn:** `[]`
...
- [ ] `[NEW]` `internal/core/domain/errors.go` — tambah `ErrGeneratedCodeInvalid`, `ErrPathEscape`, `ErrReservedName`, `ErrGoModMissing`
- [ ] `[NEW]` `internal/core/port/scanner.go` — kunci `LayoutScanner`, `LayoutReport`, `BYODetection`
- [ ] `[NEW]` `tests/harness/clitest/normalizer.go` — normalisasi golden (waktu, path, ANSI, CRLF)
- [ ] `[NEW]` `tests/e2e/compile_gate_test.go` — generate ke `t.TempDir()` lalu `go build` + `go vet` + `gofmt -l`
- [ ] **Verifikasi Batch 1:** compile-gate harus **MERAH** pada v0.3.0 (membuktikan defek #1 nyata)

### Batch 2: Golden Path Correctness
* **DependsOn:** `[Batch 1]`
* **Goal:** Jadikan compile-gate HIJAU. Seluruh 5 defek berbukti + 7 defek audit diperbaiki.
- [ ] `[MODIFY]` `internal/core/service/init_service.go` — urutan `go mod tidy`; argumen `WriteFile` tertukar; destDir
- [ ] `[MODIFY]` `internal/core/service/add_service.go` — hapus duplikasi `AddValidator`; hapus seluruh error-swallow
- [ ] `[MODIFY]` `internal/adapter/template/engine.go` — gerbang `format.Source()`
- [ ] `[MODIFY]` `internal/adapter/writer/afero_writer.go` — `errors.Is`; `SafeJoin`
- [ ] `[MODIFY]` `internal/core/domain/module.go` — nama reserved Win32
- [ ] `[MODIFY]` `.github/workflows/{ci,release}.yml` — Go 1.25, `govulncheck`, `golangci-lint`
- [ ] **Verifikasi Batch 2:** compile-gate HIJAU; `go test -race ./...` lulus; Patch Receipt dilampirkan

### Batch 3: Universal Framework Matrix
* **DependsOn:** `[Batch 2]`
* **Goal:** `--router` dan `--db` benar-benar berpengaruh pada kode yang dihasilkan.
- [ ] `[NEW]` `templates/common/main_{gin,echo,fiber,stdlib}.go.tmpl`
- [ ] `[NEW]` `templates/hexagonal/handler_http_{gin,echo,fiber,stdlib}.go.tmpl`
- [ ] `[NEW]` `templates/common/postgres_{pgx,mysql,sqlite}.go.tmpl`
- [ ] `[MODIFY]` `init_service.go` + `make_service.go` — pemilihan template berbasis manifest
- [ ] **Verifikasi Batch 3:** matriks compile-gate 5 router × 3 driver (subset 6 kombinasi di CI, 15 di nightly)

### Batch 4: Real Brownfield Engine
* **DependsOn:** `[Batch 2]` *(independen dari Batch 3)*
- [ ] `[NEW]` `internal/adapter/scanner/layout_scanner.go` — bounded WalkDir + heuristik AST
- [ ] `[NEW]` `internal/adapter/scanner/byo_detector.go` — deteksi `*pgxpool.Pool`/`*sql.DB`/`*redis.Client`
- [ ] `[MODIFY]` `internal/core/service/init_service.go` — `AdoptProject` memakai scanner, `--dry-run` default
- [ ] `[MODIFY]` `templates/hexagonal/repository_*.go.tmpl` — cabang BYO nyata, bukan komentar
- [ ] `[NEW]` `tests/fixtures/legacy_*/` — 3 repo legacy sintetis sebagai korpus uji
- [ ] **Verifikasi Batch 4:** adopt pada 3 fixture menghasilkan mapping benar; nol tulis tanpa `--apply`

### Batch 5: Anomaly Hardening & Regression Banking
* **DependsOn:** `[Batch 2]`
- [ ] `[NEW]` `tests/regression/issue_20260807_*_test.go` — satu berkas per anomali §4
- [ ] `[MODIFY]` `internal/adapter/cli/lock.go` — pelepasan lock via `defer`
- [ ] `[MODIFY]` `internal/adapter/cli/cmd_core.go` — batas walk-up `ls`; exit code terstandar
- [ ] **Verifikasi Batch 5:** 16 anomali punya test; seluruhnya HIJAU

### Batch 6: Command Tiering & Doctor Mentor
* **DependsOn:** `[Batch 3, Batch 4, Batch 5]`
- [ ] `[NEW]` `internal/core/domain/tier.go` — klasifikasi Tier-1 Verified / Experimental
- [ ] `[MODIFY]` `internal/adapter/cli/root.go` — anotasi tier di `Short`/`--help`
- [ ] `[MODIFY]` `internal/core/service/doctor_service.go` — audit Quality Gate `golang-mastery`
- [ ] **Verifikasi Batch 6:** `doctor` mendeteksi ≥8 kelas temuan pada fixture yang sengaja dirusak

### Batch 7: Release Hygiene & Patch Receipt
* **DependsOn:** `[Batch 6]`
- [ ] `[MODIFY]` `.goreleaser.yaml` — reproducible build, SBOM
- [ ] `[NEW]` `CHANGELOG.md` — entri v0.4.0 termasuk perubahan default `adopt`
- [ ] `[MODIFY]` `docs/rfc/README.md` — status RFC ini → `IMPLEMENTED`
- [ ] **Verifikasi Batch 7:** rilis kering (`goreleaser --snapshot`) lulus
