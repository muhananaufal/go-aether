...
Ketujuh batch tuntas. Status dokumen sengaja **tetap `ACCEPTED`, bukan `IMPLEMENTED`**.

Definisi `IMPLEMENTED` di skill `templates` adalah "seluruh batch task tuntas **dan lolos Quality Gate**". Quality Gate §6 `AGENTS.md` mensyaratkan `go test -race` dan linter. Keduanya belum pernah dieksekusi satu kali pun karena keterbatasan mesin di atas. Menandai dokumen ini `IMPLEMENTED` sekarang berarti mencatat klaim yang belum diukur ke dalam arsip permanen proyek — persis kelas kesalahan yang seluruh RFC ini dibuat untuk memberantas.

**Pemicu untuk mengubah menjadi `IMPLEMENTED`:** push branch → CI hijau pada ketiga job (`test` dengan `-race`, `security`, `lint`). Bila `golangci-lint` menemukan temuan di kode lama, temuan tersebut diperbaiki lebih dulu.
