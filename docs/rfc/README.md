...
- **Fase 1: Hexagonal DDD Core Engine (v0.1.0)** — Fondasi arsitektur, manifest parser, dan generator scaffolding domain, port, service, repo, handler. *(SELESAI)*
- **Fase 2: Granular Ecosystem & Middlewares (v0.2.0)** — Middleware injection (`add:middleware`), dynamic router selection, caching injection (`add:cache`). *(SELESAI)*
- **Fase 3: Distributed Systems & Observability (v0.3.0)** — Message queues/eventing (`add:worker`, `add:eventing`), metrics Prometheus (`add:metrics`), dan tracing OTel (`add:tracing`). *(SELESAI)*
